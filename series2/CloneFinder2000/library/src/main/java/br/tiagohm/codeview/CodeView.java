...
public class CodeView extends WebView {
    ...
    private int[] highlightLineNumbers = new int[]{-1};
...
//        highlightLineNumber = attributes.getInt(R.styleable.CodeView_cv_highlight_line_number, -1);
                        ...
                        if (onHighlightListener != null) {
                            new Handler(Looper.getMainLooper()).post(new Runnable() {
                                ...
                                public void run() {
                                    ...
                                    highlightLineNumber(getHighlightLineNumbers());
                                }
                            });
    ...
    public int[] getHighlightLineNumbers() {
        return highlightLineNumbers;
    }

    public void highlightLineNumber(int[] lineNumbers) {
        this.highlightLineNumbers = lineNumbers;

        for (int x : lineNumbers) {
            if (x >= 0) {
                executeJavaScript(String.format(Locale.ENGLISH,
                        "var x = document.querySelectorAll(\"td.line[line='%d']\"); if(x && x.length == 1) x[0].classList.add('destacado');", x));
            }
        }
    }

    public void highlightLineNumbers(List<Integer> numbers) {
        executeJavaScript(String.format(Locale.ENGLISH,
                "var x = document.querySelectorAll('.destacado'); if(x && x.length == 1) x[0].classList.remove('destacado');"));
        for (int lineNumber : numbers) {
            if (lineNumber >= 0) {
                executeJavaScript(String.format(Locale.ENGLISH,
